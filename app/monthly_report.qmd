...
summary_df <- data.frame(
  `Total Budget` = dollar(total_limit),
  `Total Spent` = dollar(total_spent),
  `Net Result` = paste0(dollar(abs(total_diff)), " (", status_text, ")"),
  check.names = FALSE
)
k <- kable(summary_df, format = "latex", booktabs = TRUE, align = "c") %>%
...
if (!is.null(category_summary) && !is.null(active_budgets)) {
  ...
  if (nrow(plot_summary) > 0) {
    ...
    p <- ggplot(plot_summary, aes(x = reorder(Category, Percent), y = Percent)) +
      ...
      labs(x = "Category", y = "Percent of budget", title = "Spending vs Budget (%)") +
...
if (!is.null(report_data) && nrow(report_data) > 0) {
  table_display <- report_data %>%
    ...
    mutate(
      Limit = dollar(Limit),
      Total = dollar(Total),
      Remaining = dollar(Remaining)
    )
  
  ktable <- kable(table_display, format = "latex", booktabs = TRUE, linesep = "") %>%
